...
class HyperValidator(ast.NodeVisitor):
    def __init__(self, registry: HyperRegistry, current_file: Path, resolver: HyperResolver, check_unused: bool=True):
        ...
        self.imports_map: Dict[str, Tuple[Optional[str], int, Optional[str]]] = {}
    ...
    def report_error(self, lineno: int, msg: str):
        self.errors.append((lineno, msg))
    def report_warning(self, lineno: int, msg: str):
    ...
    def _lookup_symbol(self, name: str) -> Optional[SymbolDef]:
        ...
        if self.current_mod_info and name in self.current_mod_info.star_imported_symbols:
    ...
    def visit_Name(self, node):
        if isinstance(node.ctx, ast.Load):
            ...
            if not self._is_name_defined(node.id):
                self.report_error(node.lineno, f"Name '{node.id}' is not defined")
    ...
    def visit_FunctionDef(self, node):
        ...
        for default in node.args.defaults:
            if isinstance(default, (ast.List, ast.Dict, ast.Set)):
                self.report_error(node.lineno, f"Mutable default argument in function '{node.name}'")
        ...
        for alias in node.names:
            ...
            if not self.resolver.resolvable(name):
                if self.resolver.is_internal_root(name):
                    self.report_error(node.lineno, f"Unresolvable import '{name}'")
                else:
                    self.report_warning(node.lineno, f"Module '{name}' not found")
    def visit_ImportFrom(self, node):
        mod_name = node.module
        level = node.level
        abs_mod = self._resolve_abs(mod_name, level)
        if abs_mod is None and level > 0:
            self.report_error(node.lineno, 'Relative import beyond top-level package')
            return
        ...
        if target_mod and (not self.resolver.resolvable(target_mod)):
            if self.resolver.is_internal_root(target_mod):
                self.report_error(node.lineno, f"Module '{target_mod}' not found")
            else:
                self.report_warning(node.lineno, f"Module '{target_mod}' not found")
            return
        ...
        for alias in node.names:
            ...
            self.scope_stack[-1].defined_names.add(asname)
            potential_submod = f'{target_mod}.{alias.name}' if target_mod else alias.name
            ...
            if mod_info:
                ...
                if not (is_defined or is_submodule or is_exported or is_star_imported or is_dynamic):
                    self.report_error(node.lineno, f"Cannot import '{alias.name}' from '{target_mod}'")
