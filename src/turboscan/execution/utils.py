...
Key fixes:
1. Robust @lru_cache detection and removal from classes
2. Deep recursive patching of nested objects
...
_ORIGINAL_METHODS: Dict[Tuple[int, str], Any] = {}  # (id(cls), method_name) -> original method
...
def _is_lru_cache_method(obj) -> bool:
    ...
    try:
        ...
        has_cache_info = hasattr(obj, 'cache_info') and callable(getattr(obj, 'cache_info', None))
        has_cache_clear = hasattr(obj, 'cache_clear') and callable(getattr(obj, 'cache_clear', None))
...
def patch_class_for_multiprocessing(cls: type, use_instance_cache: bool = False) -> int:
    """
...
def _get_all_referenced_classes(obj, seen: Optional[Set[int]] = None) -> Set[type]:
    """
...
def _hyperboost_cloudpickle_worker(serialized_work: bytes) -> bytes:
    ...
    try:
        ...
        func, item, key, kwargs = cloudpickle.loads(serialized_work)
        
        # Execute the function
        result = func(item, **kwargs) if kwargs else func(item)
        
        # Return success
        return cloudpickle.dumps(('success', result))
...
def restore_class_methods(cls: type) -> int:
    ...
    with _PATCH_LOCK:
        ...
        _PATCHED_CLASSES.discard(cls_id)
...
def is_class_patched(cls: type) -> bool:
    ...
    return id(cls) in _PATCHED_CLASSES
