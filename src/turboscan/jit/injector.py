...
class JITInjector:
    ...
    JIT_CANDIDATES = {'numpy', 'np', 'math', 'cmath', 'statistics', 'itertools', 'functools', 'operator'}
    NUMBA_SAFE_OPS = {ast.Add, ast.Sub, ast.Mult, ast.Div, ast.Mod, ast.Pow, ast.LShift, ast.RShift, ast.BitOr, ast.BitXor, ast.BitAnd, ast.FloorDiv, ast.MatMult, ast.UAdd, ast.USub, ast.Not, ast.Invert, ast.Eq, ast.NotEq, ast.Lt, ast.LtE, ast.Gt, ast.GtE}
    # NumPy functions that don't support scalar inputs in Numba and may cause TypingError
    NUMBA_SCALAR_INCOMPATIBLE_FUNCS = {'clip', 'reshape', 'transpose', 'ndenumerate', 'ndindex'}
    _jit_cache = {}
    ...
    def _is_numba_compatible(self, node: ast.FunctionDef) -> bool:
        if not NUMBA_AVAIL:
            return False
        INCOMPATIBLE_DECORATORS = {'property', 'contextmanager', 'asynccontextmanager', 'wraps', 'singledispatch', 'singledispatchmethod', 'abstractmethod', 'dataclass', 'pytest', 'fixture', 'mark', 'parametrize'}
        ...
        for child in ast.walk(node):
            if isinstance(child, (ast.AsyncFunctionDef, ast.Await, ast.Yield, ast.YieldFrom)):
                return False
            ...
            if isinstance(child, ast.Call) and isinstance(child.func, ast.Attribute):
                if (isinstance(child.func.value, ast.Name) and
                    ...
                    child.func.attr in self.NUMBA_SCALAR_INCOMPATIBLE_FUNCS):
                    return False
                ...
                for item in child.items:
                    ...
                    if isinstance(ctx, ast.Call):
                        if isinstance(ctx.func, ast.Attribute):
                            if ctx.func.attr in ('errstate', 'catch_warnings', 'printoptions'):
                                return False
                            if isinstance(ctx.func.value, ast.Name):
                                if ctx.func.value.id in ('np', 'numpy', 'warnings'):
                                    return False
            ...
            if isinstance(child, ast.Attribute) and isinstance(child.value, ast.Name):
                ...
                if obj_name in ('np', 'numpy', 'math', 'cmath', 'random', 'torch'):
                    continue
                if obj_name in arg_names:
                    continue
                SAFE_ATTRS = {'shape', 'dtype', 'ndim', 'size', 'T', 'real', 'imag', 'astype', 'view', 'copy', 'ravel', 'flatten', 'item', 'min', 'max', 'sum', 'mean', 'std', 'var', 'arg'}
    ...
    def _has_loop_conditional_assigns(self, node: ast.FunctionDef) -> bool:
        loop_conditional_vars = set()
        loop_used_vars = set()
        def scan_loop_body(loop_node):
            cond_vars = set()
            used_vars = set()
            def collect_in_conditional(n, in_cond=False):
    ...
    def _process_function(self, node: ast.FunctionDef):
        ...
        for dec in node.decorator_list:
            ...
            if isinstance(dec, ast.Name):
                ...
                dec_name = dec.func.id
            if dec_name in ('njit', 'jit', 'vectorize', 'guvectorize', 'cuda_jit'):
                return
        if self._is_numba_compatible(node) and (self._has_loops(node) or self._is_math_heavy(node)):
            njit_decorator = ast.Name(id='njit', ctx=ast.Load())
            use_parallel = not self._has_loop_conditional_assigns(node)
            njit_call = ast.Call(func=njit_decorator, args=[], keywords=[ast.keyword(arg='cache', value=ast.Constant(value=True)), ast.keyword(arg='fastmath', value=ast.Constant(value=True)), ast.keyword(arg='parallel', value=ast.Constant(value=use_parallel)), ast.keyword(arg='error_model', value=ast.Constant(value='python'))])
            node.decorator_list.append(njit_call)
            self.jit_count += 1
        elif self._is_vectorizable(node):
            vec_decorator = ast.Name(id='vectorize', ctx=ast.Load())
            vec_call = ast.Call(func=vec_decorator, args=[], keywords=[ast.keyword(arg='cache', value=ast.Constant(value=True))])
