...
def test_error_propagation():
    ...
    try:
        results = HyperBoost.run(failing_func, inputs, backend='threads', quiet=True)
...
def test_starmap_three_args():
    """Test starmap with three arguments."""
    inputs = [(1, 2, 3), (4, 5, 6), (7, 8, 9)]
    results = HyperBoost.starmap(multiply, inputs, backend='threads', quiet=True)
    assert results == [6, 120, 504]


def test_starmap_empty():
...
def test_mixed_types():
    ...
    inputs = [1, 2, 3, 4, 5, 6]
    results = HyperBoost.run(process_mixed, inputs, backend='threads', quiet=True)
    assert results[0] == 1
...
def test_chunking_with_workers():
...
def test_lambda_functions():
    ...
    inputs = [1, 2, 3, 4]
    try:
        results = HyperBoost.run(lambda x: x * 3, inputs, backend='threads', quiet=True)
...
def test_closure():
    ...
    inputs = [1, 2, 3, 4]
    results = HyperBoost.run(multiply_by_constant, inputs, backend='threads', quiet=True)
...
def test_min_chunks_parameter():
    """Test that min_chunks parameter is accepted."""
    inputs = [1, 2, 3, 4, 5]
    # Should work with min_chunks=None (default)
    results = HyperBoost.run(square, inputs, backend='threads', quiet=True, min_chunks=None)
    assert results == [1, 4, 9, 16, 25]
    
    # Should work with explicit min_chunks
    results = HyperBoost.run(square, inputs, backend='threads', quiet=True, min_chunks=4)
    assert results == [1, 4, 9, 16, 25]


def test_min_chunks_auto():
    """Test min_chunks='auto' for automatic scaling."""
    inputs = [1, 2, 3, 4, 5]
    results = HyperBoost.run(square, inputs, backend='threads', quiet=True, min_chunks='auto')
    assert results == [1, 4, 9, 16, 25]


def test_min_chunks_with_numpy_arrays():
    ...
    results = HyperBoost.run(sum_array, inputs, backend='threads', quiet=True, min_chunks='auto')
    # Results should be reassembled to match expected
    assert len(results) == 1


def test_min_chunks_with_lists():
    ...
    results = HyperBoost.run(sum_list, inputs, backend='threads', quiet=True, min_chunks=4)
    assert len(results) == 1


def test_min_chunks_preserves_tuple_type():
...
def test_min_chunks_invalid_value():
    """Test that invalid min_chunks values are handled gracefully."""
    inputs = [1, 2, 3]
    
    # Negative value should fall back to default
    results = HyperBoost.run(square, inputs, backend='threads', quiet=True, min_chunks=-1)
    assert results == [1, 4, 9]
    
    # Zero should fall back to default
    results = HyperBoost.run(square, inputs, backend='threads', quiet=True, min_chunks=0)
    assert results == [1, 4, 9]
...
def test_task_chaining():
    ...
    inputs = [1, 2, 3, 4]
    results = HyperBoost.run([square, double], inputs, backend='threads', quiet=True)
    # square then double: 1->1->2, 2->4->8, 3->9->18, 4->16->32
    assert results == [2, 8, 18, 32]


def test_task_chaining_three_functions():
    """Test chaining three functions."""
    inputs = [1, 2, 3]
    results = HyperBoost.run([increment, square, double], inputs, backend='threads', quiet=True)
    # increment then square then double: 1->2->4->8, 2->3->9->18, 3->4->16->32
    assert results == [8, 18, 32]


def test_task_chaining_empty():
...
def test_boost_all_single_task():
...
def test_recursive_parallelization_prevented():
    ...
    def nested_run(x):
        ...
        inner_results = HyperBoost.run(square, [x], backend='threads', quiet=True)
...
def test_caching_enabled():
    ...
    # First run
    results1 = HyperBoost.run(counting_square, inputs, backend='threads', quiet=True, use_cache=True)
    first_count = call_count[0]
    
    # Second run with same inputs should use cache
    results2 = HyperBoost.run(counting_square, inputs, backend='threads', quiet=True, use_cache=True)
...
def test_caching_disabled():
    ...
    inputs = [1, 2, 3]
    results = HyperBoost.run(square, inputs, backend='threads', quiet=True, use_cache=False)
    assert results == [1, 4, 9]
...
def test_custom_batch_size():
    ...
    inputs = list(range(100))
    results = HyperBoost.run(square, inputs, backend='threads', quiet=True, batch_size=10)
    expected = [x * x for x in inputs]
    assert results == expected
...
def test_batch_size_larger_than_input():
    ...
    inputs = [1, 2, 3]
    results = HyperBoost.run(square, inputs, backend='threads', quiet=True, batch_size=100)
    assert results == [1, 4, 9]
...
def test_force_processes():
    """Test force_processes parameter."""
    inputs = [1, 2, 3, 4, 5]
    results = HyperBoost.run(square, inputs, backend='auto', quiet=True, force_processes=True)
    assert results == [1, 4, 9, 16, 25]
...
def test_nested_list_processing():
...
def test_get_effective_min_chunks_auto():
...
def test_get_effective_min_chunks_int():
...
def test_get_effective_min_chunks_invalid():
...
def test_is_chunkable_item_list():
...
def test_is_chunkable_item_tuple():
...
def test_is_chunkable_item_numpy():
...
def test_is_chunkable_item_scalar():
...
def test_subdivide_items_with_lists():
    """Test _subdivide_items with lists."""
    items = [list(range(20))]
    result, chunk_map = HyperBoost._subdivide_items(items, 4)
    assert len(result) >= 1


def test_subdivide_items_with_tuples():
...
def test_subdivide_items_non_chunkable():
    """Test _subdivide_items with non-chunkable items."""
    items = [1, 2, 3]
    result, chunk_map = HyperBoost._subdivide_items(items, 10)
    assert len(result) == 3  # Can't subdivide scalars


def test_reassemble_results_no_subdivision():
...
def test_reassemble_results_with_numpy():
...
def test_reassemble_results_with_lists():
...
def test_reassemble_results_with_2d_numpy():
...
def test_cpu_bound_detection():
    ...
    results = HyperBoost.run(cpu_intensive, [1, 2, 3], backend='threads', quiet=True)
...
def test_processes_backend_simple():
    """Test processes backend with simple function."""
    def simple_square(x):
        return x * x
    
    inputs = [1, 2, 3, 4, 5]
    results = HyperBoost.run(simple_square, inputs, backend='processes', quiet=True)
    assert results == [1, 4, 9, 16, 25]


def test_processes_backend_with_imports():
...
def test_partial_failure():
    ...
    with pytest.raises(ValueError):
        HyperBoost.run(failing_func, [1, 2, 3, 4], backend='threads', quiet=True)
...
def test_large_result_handling():
    ...
    inputs = [1, 2, 3]
    results = HyperBoost.run(create_large, inputs, backend='threads', quiet=True)
    assert len(results) == 3
...
def test_actual_parallelism():
    ...
    start = time.time()
    results = HyperBoost.run(slow_func, [1, 2, 3, 4], backend='threads', quiet=True)
