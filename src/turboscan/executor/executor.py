...
class HyperExecutor:
    def __init__(self, script_path: str, audit_first: bool=True, optimize: bool=True, inject_jit: bool=True):
        self.script_path = Path(script_path).resolve()
        self.audit_first = audit_first
        self.optimize = optimize
        self.inject_jit = inject_jit
        self.stats = {'parallelized_loops': 0, 'parallelized_tasks': 0, 'jit_functions': 0, 'vectorized_functions': 0, 'audit_errors': 0, 'audit_warnings': 0}
    def _run_audit(self) -> bool:
        project_root = self.script_path.parent
        auditor = HyperAuditor(project_root, excludes={'.git', 'node_modules', 'venv', '__pycache__', 'env', '.venv', 'build', 'dist'}, check_unused=True)
    ...
    def _prepare_globals(self) -> Dict[str, Any]:
        global_env = {'__name__': '__main__', '__file__': str(self.script_path), '__builtins__': builtins, 'HyperBoost': HyperBoost, 'Boost': HyperBoost, 'GPU_ACCELERATOR': GPU_ACCELERATOR, 'HYPER_CACHE': HYPER_CACHE, 'HARDWARE': HARDWARE, 'prange': prange}
        if NUMBA_AVAIL:
    ...
    def execute(self):
        ...
        if RICH_AVAIL:
            ...
            console.print(Panel(f'[bold green]⚡ TurboScan HYPER Execution[/bold green]\n[dim]Script: {self.script_path.name}[/dim]\n[dim]CPUs: {HARDWARE.cpu_count} | GPUs: {GPU_COUNT}[/dim]\n[dim]Serializer: {serializer}[/dim]', border_style='green'))
        else:
            serializer = 'cloudpickle ✓' if CLOUDPICKLE_AVAIL else 'pickle (consider: pip install cloudpickle)'
        ...
        try:
            ...
            compiled_code = compile(tree, filename=str(self.script_path), mode='exec')
            if RICH_AVAIL:
                table = Table(title='Optimization Results')
                table.add_column('Metric', style='cyan')
                table.add_column('Count', style='green')
                table.add_row('Parallelized Loops', str(self.stats['parallelized_loops']))
                table.add_row('Parallelized Tasks', str(self.stats['parallelized_tasks']))
                table.add_row('JIT Functions', str(self.stats['jit_functions']))
                table.add_row('Vectorized Functions', str(self.stats['vectorized_functions']))
                console.print(table)
            else:
                print(f"   Parallelized: {self.stats['parallelized_loops']} loops, {self.stats['parallelized_tasks']} tasks")
                print(f"   JIT: {self.stats['jit_functions']} functions, {self.stats['vectorized_functions']} vectorized")
        except SyntaxError as e:
            print(f'Syntax error at line {e.lineno}: {e.msg}')
            sys.exit(1)
        except Exception as e:
            print(f'⚠️  Optimization failed ({e}). Running unoptimized.')
            compiled_code = compile(source_code, filename=str(self.script_path), mode='exec')
        print('\n[Phase 3/3] Executing...')
        print('=' * 60)
        ...
        if cache_stats['hits'] + cache_stats['misses'] > 0:
            print(f"   Cache: {cache_stats['hit_rate']:.1%} hit rate ({cache_stats['hits']} hits, {cache_stats['misses']} misses)")
