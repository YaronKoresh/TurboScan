"""Ultra-fast parallel project auditing with validation and import checking."""
...
class HyperAuditor:
    def __init__(
        self, root: Path, excludes: Set[str], check_unused: bool = True
    ) -> None:
        self.root = root
    ...
    def _validate_file(self, file_path: Path) -> List[str]:
        ...
        try:
            ...
            validator = HyperValidator(
                self.registry, file_path, self.resolver, self.check_unused
            )
            ...
            if self.check_unused:
                ...
                for u in sorted(unused):
                    issues.append(
                        f"[UNUSED] {rel} -> Imported '{u}' is never used"
                    )
            return issues
        except SyntaxError as e:
            return [
                f"[ERROR] {file_path}: Syntax error at line {e.lineno}: {e.msg}"
            ]
    ...
    def run(self) -> Tuple[int, int]:
        ...
        all_results = HyperBoost.run(
            self._validate_file, sorted_files, quiet=False, backend="auto"
        )
        flat_issues = []
