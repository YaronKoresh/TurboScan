...
class TestHyperExecutor:
    ...
    def test_execute_flow(self, tmp_path):
        ...
        with patch('turboscan.executor.executor.HyperAuditor') as MockAuditor, \
             patch('turboscan.executor.executor.HyperBoost') as MockBoost, \
             patch('turboscan.executor.executor.JITInjector') as MockJIT, \
             patch('turboscan.executor.executor.HyperAutoParallelizer') as MockParallelizer, \
             patch('turboscan.executor.executor.FAST_READER') as MockReader, \
             patch('turboscan.executor.executor.NUMBA_AVAIL', True):
            ...
            # Initialize & Run
            executor = HyperExecutor(str(script_file), audit_first=True, optimize=True)
            executor.execute()
    ...
    def test_execute_stops_on_audit_failure(self, tmp_path):
        ...
        with patch('turboscan.executor.executor.HyperAuditor') as MockAuditor, \
             ...
             patch('turboscan.executor.executor.RICH_AVAIL', False):
