"""Function analysis for optimal execution strategy selection."""
...
class FunctionAnalysis:
    ...
    def should_force_threads(self) -> bool:
        ...
        if self.is_lambda and (not CLOUDPICKLE_AVAIL):
            return True
        return bool(self.is_local_func and not CLOUDPICKLE_AVAIL)


class FunctionAnalyzer:
    THREAD_PATTERNS = frozenset(
        {
            "ThreadPoolExecutor",
            "threading.Thread",
            "Thread(",
    ...
    ASYNC_PATTERNS = frozenset(
        {"async def", "await ", "asyncio.", "aiohttp", "aiofiles"}
    )
    CPU_BOUND_PATTERNS = frozenset(
        {
            "numpy",
            "np.",
            "scipy",
    ...
    def analyze(cls, func: Callable) -> FunctionAnalysis:
        ...
        analysis.__turbo_id__ = f"analysis:{func_id}"
        ...
        if hasattr(func, "__closure__") and func.__closure__:
            ...
            for cell in func.__closure__:
                try:
                    ...
                    if (
                        cell_type in cls.THREAD_NAMES
                        or "threading" in cell_module
                        or "concurrent" in cell_module
                        or cell_module == "_thread"
                    ):
        ...
        if hasattr(func, "__code__"):
            code = func.__code__
            code_names = (
                set(code.co_names) if hasattr(code, "co_names") else set()
            )
            free_vars = (
                set(code.co_freevars) if hasattr(code, "co_freevars") else set()
            )
            ...
            try:
                ...
                if mod:
                    ...
                    if mod_source and os.path.exists(mod_source):
                        try:
                            with open(
                                mod_source, encoding="utf-8", errors="ignore"
                            ) as f:
                                mod_content = f.read()
                            if any(
                                p in mod_content for p in cls.THREAD_PATTERNS
                            ):
                                analysis.confidence = max(
                                    0.3, analysis.confidence - 0.2
                                )
                        except Exception:
                            pass
            except Exception:
    ...
    def _test_pickle(cls, func: Callable) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            return (False, f"Unknown: {type(e).__name__}: {e!s}")
