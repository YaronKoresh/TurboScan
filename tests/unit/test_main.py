...
class TestTurboScanMain:
    ...
    def mock_deps(self):
        ...
        with patch('turboscan.__main__.HyperExecutor') as mock_exec, \
             patch('turboscan.__main__.HyperAuditor') as mock_audit, \
             patch('turboscan.__main__.console') as mock_console, \
             patch('turboscan.__main__.HARDWARE') as mock_hardware, \
             patch('turboscan.__main__.HyperBoost') as mock_boost:
    ...
    def test_info_command_with_rich(self, mock_deps):
        """Test 'info' command when Rich is available."""
        _, _, mock_console, _, _ = mock_deps
        
        # Simulate 'turboscan info'
        with patch.object(sys, 'argv', ['turboscan', 'info']), \
             patch('turboscan.__main__.RICH_AVAIL', True), \
            ...
            assert "Table" in str(type(args[0])) # Verify it printed a Table object

    def test_info_command_without_rich(self, mock_deps):
        """Test 'info' command when Rich is NOT available (fallback to print)."""
        _, _, _, _, _ = mock_deps
        
        # Capture standard output
        with patch.object(sys, 'argv', ['turboscan', 'info']), \
             patch('turboscan.__main__.RICH_AVAIL', False), \
    ...
    def test_default_command_is_info(self, mock_deps):
        """Running without arguments should default to 'info' behavior."""
        _, _, mock_console, _, _ = mock_deps
        
        with patch.object(sys, 'argv', ['turboscan']), \
             patch('turboscan.__main__.RICH_AVAIL', True):
    ...
    def test_audit_command_success(self, mock_deps):
        """Test 'audit' command with successful result."""
        _, mock_audit_cls, _, _, _ = mock_deps
        
        # Mock auditor returning 0 errors
        mock_audit_cls.return_value.run.return_value = (0, 5) # 0 errors, 5 warnings
        
        with patch.object(sys, 'argv', ['turboscan', 'audit', './src', '--exclude', 'temp']), \
    ...
    def test_audit_command_failure(self, mock_deps):
        ...
        with patch.object(sys, 'argv', ['turboscan', 'audit', '--no-unused']), \
             pytest.raises(SystemExit) as exc:
    ...
    def test_run_command_defaults(self, mock_deps):
        """Test 'run' command with default arguments."""
        mock_exec_cls, _, _, _, _ = mock_deps
        
        with patch.object(sys, 'argv', ['turboscan', 'run', 'script.py', 'arg1']):
            main()
    ...
    def test_run_command_flags(self, mock_deps):
