...
class TestFastFileReader:
    ...
    def test_read_missing_file_returns_empty(self, reader) -> None:
        """Missing files should return empty string, not crash."""
        assert reader.read_file(Path("non_existent_ghost.py")) == ""

    def test_read_files_parallel(self, reader, tmp_path) -> None:
    ...
    @pytest.mark.skipif(
        not PYTEST_ASYNCIO_AVAILABLE, reason="pytest-asyncio not available"
    )
    @pytest.mark.asyncio
    async def test_read_files_async(self, reader, tmp_path) -> None:
