...
class TestHyperAuditor:
    ...
    def mock_deps(self):
        ...
        with patch('turboscan.auditor.auditor.HyperRegistry') as reg, \
             patch('turboscan.auditor.auditor.HyperResolver') as res, \
             patch('turboscan.auditor.auditor.HyperValidator') as val, \
             patch('turboscan.auditor.auditor.HyperBoost') as boost, \
             patch('turboscan.auditor.auditor.FAST_READER') as reader:
    ...
    def test_init_scans_registry(self, mock_deps):
        """Auditor should initialize and scan the registry."""
        reg_cls, res_cls, _, _, _ = mock_deps
        root = Path('/project')
    ...
    def test_validate_file_valid(self, mock_deps):
    ...
    def test_validate_file_detects_issues(self, mock_deps):
    ...
    def test_run_counts_errors(self, mock_deps):
