...
class HyperResolver:
    def __init__(self, registry: HyperRegistry) -> None:
    ...
    def _discover_site_packages(self) -> List[Path]:
        ...
        for p in sys.path:
            ...
            seen.add(pp)
            if (
                "site-packages" in pp.parts or "dist-packages" in pp.parts
            ) and pp.exists():
                paths.append(pp)
        return paths

    def _scan_external_toplevel(self) -> None:
        for sp in self.site_packages:
            ...
            try:
                for item in sp.iterdir():
                    name = item.name
                    if item.is_dir():
                        if name.endswith(".dist-info") or name.endswith(
                            ".egg-info"
                        ):
    ...
    def _internal_exists(self, fqn: str) -> bool:
        ...
        if self.registry.src_prefix:
            mod_path = (
                self.registry.root / self.registry.src_prefix / Path(*parts)
            )
        else:
            mod_path = self.registry.root / Path(*parts)
        if mod_path.with_suffix(".py").exists():
            return True
        return bool(mod_path.exists() and mod_path.is_dir())

    def _stdlib_exists(self, fqn: str) -> bool:
        top = fqn.split(".", maxsplit=1)[0]
    ...
    def _external_exists(self, fqn: str) -> bool:
        top = fqn.split(".", maxsplit=1)[0].lower()
    ...
    def resolvable(self, fqn: str) -> bool:
        ...
        ok = (
            self._internal_exists(fqn)
            or self._stdlib_exists(fqn)
            or self._external_exists(fqn)
        )
        self.memo[fqn] = ok
        return ok
    ...
    def is_internal_root(self, fqn: str) -> bool:
        ...
        return any(
            m == root or m.startswith(root + ".") for m in self.registry.modules
        )
